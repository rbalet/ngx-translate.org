...
:::note
Still on v17? See the [v17 installation guide](/v17/getting-started/installation/).
:::
...
For standalone applications, the recommended way to initialize the `TranslateService` is with
[`provideAppInitializer`](https://angular.dev/api/core/provideAppInitializer) in your `appConfig`.
This runs once during application bootstrap, before your components are rendered, keeping side
effects out of your components and avoiding any flicker of untranslated content:

```ts {1-3, 10-15} title="app.config.ts"
...
Alternatively, you can access the `TranslateService` in a component using the modern `inject()`
function and initialize it in the `AppComponent` constructor. You'll need to import `TranslatePipe`
and `TranslateDirective` into your component's `imports` (v18 also ships
[`TranslateBlockDirective`](/getting-started/translating-your-components/#translateblock-structural-directive)
for the `*translateBlock` structural directive — import it when you need it):
